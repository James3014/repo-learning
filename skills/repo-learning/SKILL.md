...
12. Prefer progressive cue fading and later transfer in a materially different real case. Do not repeatedly ask the same definition or re-quiz a judgment the user already expressed. A learning event records evidence facts; it does not grant itself cue-suppression authority. Suppress a proactive prompt only when the bounded state derives a still-current fading window from user-authored, pre-evidence, no-cue, delayed (>=24h), independent near/far transfer evidence. Legacy `silent_cue_fading_eligible` event flags are not authority. When the bounded fading window expires, allow natural revalidation. Chat memory alone is insufficient. Plain-language recognition of a system risk may be stronger evidence than terminology recall.
...
- A single interaction may have multiple defects; preserve all applicable defect classes in dogfood observation instead of hiding them behind one precedence winner.
- Self-reported semantic labels may support debugging but are not independent G5 interaction-quality evidence. Bind evaluated interactions to a privacy-safe user-visible trace hash and record evaluator provenance.
- An evidence event must not directly authorize its own future cue suppression; cue fading is derived policy with a bounded validity window.
- Prompt language should align with the user's current language unless they request otherwise; exact repository identifiers may remain unchanged.
...
Read [references/contract-manifest.json](references/contract-manifest.json) when producing or validating dogfood/rollout evidence so the loaded Skill contract is attested.
