...
3. Decide whether there is a real, high-value architecture judgment point. Many tasks should produce no learning interaction.
4. Suppress proactive teaching for mechanical, urgent, exact-machine-output, or already-revealed work.
5. In `guided` mode, surface at most one primary judgment point. In `observe`, do not proactively quiz. In `practice`, allow stronger learning load without taking engineering control.
...
- Learning unavailable must degrade to normal engineering, not block engineering.
