...
9. If the user does not answer a visible `JUDGMENT_PROMPT`, classify it as `skip/no-response` and continue normal engineering without waiting, grading, inferring regression, or lowering mastery. Do not emit a second prompt for the same selected point.
10. In `observe`, do not proactively quiz. In `practice`, allow stronger learning load without taking engineering control.
11. Evaluate only the user's own reasoning using goals, constraints, alternatives, trade-offs, evidence and falsifiers. Agreement with the AI is not the scoring rule. For `SPONTANEOUS_JUDGMENT_CAPTURE`, only reasoning expressed before RepoLearn feedback or decisive evidence may count as candidate mastery evidence; the AI's reformulation, naming, or explanation is exposure, not user evidence.
12. Prefer progressive cue fading and later transfer in a materially different real case. Do not repeatedly ask the same definition or re-quiz a judgment the user already expressed. Plain-language recognition of a system risk may be stronger evidence than terminology recall.
...
- A guided prompt that appears only after answer-revealing recommendation, conclusion, or decisive evidence is a pre-evidence-ordering defect, not `skip/no-response`, not negative user evidence, and not mastery regression.
